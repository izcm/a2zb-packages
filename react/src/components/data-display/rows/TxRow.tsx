...
export type TxRowStatus = "pending" | "success" | "failed";
...
type Props = {
  tx: TxRowItem;
  /** Overrides the default "etherscan" link text for a given tx label, e.g. { "order filled": "view trade" }. */
  linkLabels?: Record<string, string>;
  onClick?: () => void;
  disabled?: boolean;
};

export function TxRow({ tx, linkLabels, onClick, disabled }: Props) {
...
function StatusIcon({ status }: { status: TxRowStatus }) {
  if (status === "success") {
    return (
      <svg
