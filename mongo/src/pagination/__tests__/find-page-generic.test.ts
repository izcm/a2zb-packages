...
describe("findPageGeneric", () => {
  ...
  describe("query building", () => {
    const givenCursorFilter = (cf: any) =>
      vi.mocked(buildCursorFilter).mockReturnValueOnce(cf);
    ...
    it.each([
      ...
      "merges cursor filter into $and correctly (%s)",
      async (_, baseQuery, expectedQuery) => {
  ...
  describe("pagination result", () => {
    ...
    it("handles limit 0 by throwing error", async () => {
      await expect(runPage({ args: { limit: 0 } })).rejects.toThrow(
