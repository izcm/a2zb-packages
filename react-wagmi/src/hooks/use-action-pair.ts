...
> = {
  readAction: Omit<ReadAction<TReadAbi, TReadFunc>, "address"> & {
    address: Address | undefined;
  };
  writeAction: Omit<
    WriteAction<TWriteAbi, TWriteFunc>,
    "address" | "args" | "value"
  > & {
    address: Address | undefined;
  };
...
>({
  readAction,
  writeAction,
}: ActionPair<TReadAbi, TReadFunc, TWriteAbi, TWriteFunc>) {
  ...
  const { data, refetch } = useReadContract({
    ...
    query: {
      enabled:
        (readAction.args as unknown[]).every((arg) => arg !== undefined) &&
        !!readAction.address,
    },
  } as Parameters<typeof useReadContract>[0]) as {
    data:
      | ContractFunctionReturnType<TReadAbi, "pure" | "view", TReadFunc>
      | undefined;
    refetch: () => void;
  };
  ...
  function write(
    args: WriteAction<TWriteAbi, TWriteFunc>["args"],
    value?: bigint,
  ) {
