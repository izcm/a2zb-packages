...
export function isRetryable(err: unknown): boolean {
  ...
  if (err && typeof err === 'object' && 'code' in err && typeof err.code === 'number') {
    return err.code === -1 || err.code === -32603 || err.code === -32005 || err.code === 429
  }
  ...
  if (err instanceof Error) {
    return /ECONNRESET|ENOTFOUND|EAI_AGAIN|ETIMEDOUT|ECONNREFUSED|fetch failed/i.test(err.message)
  }
  return false
}
